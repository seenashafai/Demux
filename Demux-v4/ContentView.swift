...
//            .edgesIgnoringSafeArea(.all)
        VStack {
            WelcomeText()
            Button(action: {
                let aT = appdelegate.sessionManager.debugDescription
                
                print(aT, "auth")
                //Send req to API to get user details
                AF.request(userInfoEndpoint, method: .get, encoding: JSONEncoding.default, headers: ["Authorization": "Bearer "+aT]).responseJSON {response in
